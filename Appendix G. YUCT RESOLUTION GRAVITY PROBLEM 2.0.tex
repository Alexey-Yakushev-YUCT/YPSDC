...
March 2026 \\ \large V.2.0.
	...
	Here, $\alpha_{\text{YUCT}}$ is a single universal parameter (expected to be small, $\sim 10^{-3}$), $f_0$ is a reference frequency (taken as the fundamental ringdown frequency of a $30 M_\odot$ black hole), and the exponent combination $\beta\gamma \approx 0.20$ is already constrained by independent astrophysical data (white dwarfs, Earth-Moon system) . The key point is that $\alpha_{\text{YUCT}}$ is the \textit{same} for all events. If YUCT is correct, analyzing the full population of gravitational-wave detections should yield a consistent, non-zero value of $\alpha_{\text{YUCT}}$ that scales with mass as predicted.
...
\end{table}
	...
	\begin{enumerate}
		...
		\item \textbf{Experimental verification:} Laboratory and astrophysical tests, including upcoming O4b and O5 gravitational-wave data
