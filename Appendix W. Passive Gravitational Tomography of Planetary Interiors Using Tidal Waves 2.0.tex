...
\begin{document}
	...
	To assess the practical feasibility of the method, it is necessary to demonstrate that the amplitude of the tidal resonance from a typical oil and gas reservoir exceeds the sensitivity threshold of modern gravimeters (\( \sim 10^{-11} g\)). Consider a reservoir with thickness \(h = 50\) m, saturated with gas (\(\Keff \approx 6.1\)), located at a depth of \(H = 2000\) m in a homogeneous host rock with \(\Keff \approx 4.0\). The contrast in coordination effectiveness \(\Delta \Keff / \Keff \approx 0.35\) creates a corresponding contrast in density and stiffness, which can be estimated through the equation of state.
	...
	The obtained value \(\delta g_{\text{res}} \approx 3 \times 10^{-11} g\) is at the sensitivity limit of modern superconducting gravimeters such as the iGrav (\(10^{-11} g\)) and could be detected by stacking the signal over several tidal cycles. For a water-saturated reservoir (\(\Keff \approx 4.5\)), the signal would be approximately half as large (\( \approx 1.5\times10^{-11} g\)), but still potentially measurable. Thus, the direct registration of tidal resonances from deep reservoirs is at the edge of current technological capabilities, which explains the absence of reliable observations to date.
	...
	\begin{itemize}
		...
		\item Deploy a network of 15–20 superconducting gravimeters (e.g., iGrav type, sensitivity $10^{-11} g$) and broadband seismometers over a 100×100 km area (station spacing ~25–30 km).
